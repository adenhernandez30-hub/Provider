...
class AniLabGenericEmbedExtractor(
        ...
        repeat(3) { depth ->
            ...
            client.newCall(request).execute().use { response ->
                ...
                MEDIA.findAll(html).forEach { match ->
                    ...
                    result.putIfAbsent(media, AniLabStreamCandidate("", id, media, typeOf(media), referer = current, headers = context.headers, cookies = context.cookies))
                }
                val embed = EMBED.find(html)?.groupValues?.get(1)?.let { resolve(current, htmlDecode(it)) }
                if (embed != null && embed != current) current = embed else return@repeat
            }
        }
        return result.values.toList()
    }

    private fun resolve(base: String, value: String): String? = runCatching { URI(base).resolve(value).toString() }.getOrNull()?.takeIf { it.startsWith("http://") || it.startsWith("https://") }
    ...
    companion object {
        private val HOSTS = setOf("blogspot.com", "blogger.com", "filedon.co", "filedon.to", "uservideo.net", "userdrive.co", "samevideo.to", "vidhide.com", "mp4upload.com", "yourupload.com", "yuplod.net", "streamwish.to", "filelions.to", "desustream.info", "desudrive.com", "odstream.xyz", "odcdn.com", "otakuwatch.com")
        private val MEDIA = Regex("""(?:data-src|data-video|src|href)=[\"']([^\"']+\\.(?:m3u8|mpd|mp4|webm)(?:\\?[^\"']*)?)[\"']""", RegexOption.IGNORE_CASE)
        private val EMBED = Regex("""(?:data-src|data-embed|src)=[\"']([^\"']+)[\"']""", RegexOption.IGNORE_CASE)
    }
}