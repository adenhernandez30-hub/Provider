...
/** Robust multi-gateway adapter used to isolate shared remote-source failures. */
class RemoteSourceProviderV2(
    ...
    override suspend fun search(query: String): List<ProviderAnime> {
        for (url in searchUrls(query)) {
            ...
            extractItems(root).mapNotNull { it.toAnime() }.takeIf { it.isNotEmpty() }?.let { return it }
        }
        return emptyList()
    }
    ...
    override suspend fun getAnime(animeId: String): ProviderAnime? {
        ...
        for (url in detailUrls(slug)) requestJson(url)?.let { it.toAnime(slug)?.let { anime -> return anime } }
        return null
    }
    ...
    private suspend fun requestJson(url: String): JSONObject? = withContext(Dispatchers.IO) {
        runCatching {
            ...
            client.newCall(request).execute().use { response ->
                ...
                runCatching { JSONObject(body) }.getOrElse { JSONObject().put("_root", JSONArray(body)) }
            }
        }.getOrNull()
    }

    private fun extractItems(root: JSONObject): List<JSONObject> {
        root.optJSONArray("_root")?.let { return it.objects() }
        for (key in listOf("results", "anime", "items", "search", "list", "animeList", "data")) root.optJSONArray(key)?.let { return it.objects() }
        root.optJSONObject("data")?.let { nested -> extractItems(nested).takeIf { it.isNotEmpty() }?.let { return it } }
    ...
    private fun extractEpisodeItems(root: JSONObject): List<JSONObject> {
        for (key in listOf("episodes", "episode", "episodeList", "episode_list", "episodeListData")) root.optJSONArray(key)?.let { return it.objects() }
        root.optJSONObject("data")?.let { nested -> extractEpisodeItems(nested).takeIf { it.isNotEmpty() }?.let { return it } }
        return emptyList()
    }
    ...
    private fun JSONObject.toAnime(fallbackId: String? = null): ProviderAnime? {
        ...
        return ProviderAnime(
            id = "$id:$rawId", title = title, providerId = id,
    ...
    private fun collectStreams(value: Any?, out: MutableList<ProviderStream>, quality: String? = null) {
        when (value) {
            is JSONObject -> {
                ...
                val keys = value.keys(); while (keys.hasNext()) { val key = keys.next(); if (key !in directKeys) collectStreams(value.opt(key), out, nextQuality) }
            }
