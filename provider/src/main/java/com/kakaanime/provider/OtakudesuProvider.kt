...
class OtakudesuProvider(browserResolver: BrowserStreamResolver? = null) : AnimeProvider {
    ...
    private suspend fun requestJson(url: String): JSONObject? = withContext(Dispatchers.IO) { runCatching { Request.Builder().url(url).header("User-Agent", "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36").header("Accept", "application/json,text/plain,*/*").header("Accept-Language", "id-ID,id;q=0.9,en-US;q=0.8").build().let { request -> client.newCall(request).execute().use { response -> if (!response.isSuccessful) null else response.body?.string()?.takeIf { it.isNotBlank() }?.let { body -> JSONObject(body) } } } }.getOrNull() }
    private fun JSONArray.toProviderAnimeList(): List<ProviderAnime> = buildList<ProviderAnime> { for (i in 0 until length()) { val item = optJSONObject(i) ?: continue; val slug = normalizeAnimeSlug(item.optString("slug")); if (slug.isNotBlank()) add(ProviderAnime("$id:$slug", item.optString("title").ifBlank { "Unknown Anime" }, id, posterUrl = item.optString("thumbnail").ifBlank { null }, status = item.optString("status").ifBlank { "UNKNOWN" }, rating = item.optString("score").toDoubleOrNull())) } }
    private fun JSONArray.toLegacyProviderAnimeList(): List<ProviderAnime> = buildList<ProviderAnime> { for (i in 0 until length()) { val item = optJSONObject(i) ?: continue; val slug = normalizeAnimeSlug(item.optString("endpoint")); if (slug.isNotBlank()) add(ProviderAnime("$id:$slug", item.optString("title").ifBlank { "Unknown Anime" }, id, posterUrl = item.optString("thumb").ifBlank { null }, status = item.optString("status").ifBlank { "UNKNOWN" }, rating = item.optString("rating").toDoubleOrNull())) } }
    ...
    private fun JSONObject.episodeArrays(): List<JSONArray> = listOf<JSONArray>("episodeList", "episode_list", "episodes", "episodeLists").mapNotNull { optJSONArray(it) }
    private fun JSONArray.toCommunityProviderEpisodeList(slug: String): List<ProviderEpisode> = buildList<ProviderEpisode> { for (i in 0 until length()) { val item = optJSONObject(i) ?: continue; val endpoint = item.optString("slug").trim('/'); val number = item.optInt("episode_number", -1).takeIf { it >= 0 } ?: extractEpisodeNumber(item.optString("episode"), endpoint) ?: continue; if (endpoint.isBlank()) continue; add(ProviderEpisode("$id:community:$endpoint", "$id:$slug", number, id, item.optString("episode").ifBlank { "Episode $number" })) } }.sortedBy { it.number }
    private fun JSONArray.toProviderEpisodeList(slug: String): List<ProviderEpisode> = buildList<ProviderEpisode> { for (i in 0 until length()) { val item = optJSONObject(i) ?: continue; val endpoint = item.optString("slug").trim('/'); val rawNumber = item.optString("episode").trim(); val number = rawNumber.toIntOrNull() ?: item.optInt("episode_number", -1).takeIf { it >= 0 } ?: extractEpisodeNumber(item.optString("title"), endpoint) ?: continue; if (endpoint.isBlank()) continue; add(ProviderEpisode("$id:$endpoint", "$id:$slug", number, id, item.optString("title").ifBlank { "Episode $number" }, item.optString("thumbnail").ifBlank { null })) } }.sortedBy { it.number }
    private fun JSONArray.toLegacyProviderEpisodeList(slug: String): List<ProviderEpisode> = buildList<ProviderEpisode> { for (i in 0 until length()) { val item = optJSONObject(i) ?: continue; val endpoint = item.optString("endpoint").trim('/'); val number = extractEpisodeNumber(item.optString("title"), endpoint) ?: continue; add(ProviderEpisode("$id:$endpoint", "$id:$slug", number, id, item.optString("title").ifBlank { "Episode $number" }, item.optString("thumb").ifBlank { null })) } }.sortedBy { it.number }
    private fun JSONObject.streamCandidates(): List<String> = buildList<String> { val streams = optJSONArray("streams") ?: JSONArray(); for (i in 0 until streams.length()) streams.optJSONObject(i)?.optString("embedUrl")?.trim()?.takeIf { it.isNotBlank() }?.let(::add); optString("defaultStreamUrl").trim().takeIf { it.isNotBlank() }?.let(::add) }.distinct()
